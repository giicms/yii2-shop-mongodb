...
use frontend\modules\upload\components\Resize;
        ...
        for ($i = 0; $i < $fileCount; $i++) {
            ...
            if (move_uploaded_file($_FILES["myfile"]["tmp_name"][$i], $output_dir . $new_name)) {
                ...
                if ($width > 400)
                    ...
                    $scale = $w / $w;
                $cropped = Resize::resizeThumbnailImage($output_dir . 'thumbs/' . $new_name . '.png', $output_dir . $new_name . '.' . $ext, $w, $h, $x1, $y1, $scale);
//                $data[] = ['url' => Yii::$app->setting->value("url_file") . 'thumbnails/' . $new_name . '.png', 'size' => $size, 'name' => $new_name . '.png', 'img_id' => $new_name];
            }
        }
    }
